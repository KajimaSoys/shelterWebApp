  ...
  <div class="animal-list" v-if="animals.length > 0">
    ...
    <div class="main-content">
      <div class="filters">
        ...
        <!-- Shelter Filter -->
        <div>
          <label class="filter-label">Приют:</label>
          <div style="margin-top: 1rem">
            <el-select v-model="selectedShelter" placeholder="Выберите приют" clearable>
              <el-option
                v-for="type in shelterList"
                :key="type"
                :label="type"
                :value="type">
              </el-option>
...
<!--        <el-button type="primary" @click="getAnimals">Apply</el-button>-->
        ...
        <el-card v-for="animal in paginatedAnimals" :key="animal.id" @click="goToAnimal(animal.shelter, animal.id)" class="animal-card">
          ...
          <div class="animal-card-content">
            <h2 class="animal-card-title">
              {{ animal.name }}
              <el-button v-if="isShelterOwner(animal)"  @click.stop="editAnimal(animal.shelter, animal.id)">
                <el-icon>
                  <Edit />
                </el-icon>
              </el-button>
              <el-button v-if="isShelterOwner(animal)" @click.stop="deleteAnimal(animal.shelter, animal.id)">
                <el-icon>
                  <Delete />
                </el-icon>
              </el-button>
  ...
  <div class="animal-list" v-else>
    <h1>Приюты пока не добавили животных..</h1>
  </div>
</template>
...
export default {
  ...
  computed: {
    filteredAnimals() {
      ...
      if (this.selectedShelter) {
        result = result.filter(animal => animal.shelter_name === this.selectedShelter);
      }
    ...
    shelterList() {
      return [...new Set(this.animals.map(animal => animal.shelter_name))];
    },
    animalTypeList() {
  ...
  methods: {
    ...
    isShelterOwner(animal) {
      return this.user && animal.shelter_owner === this.user.id;
    },
